...
fn lua_to_resp(val: &mlua::Value, out: &mut BytesMut) {
    match val {
        ...
        mlua::Value::Boolean(true) => {
            crate::vendor::lux::resp::write_integer(out, 1);
        }
        mlua::Value::Integer(n) => {
            crate::vendor::lux::resp::write_integer(out, *n);
        }
        mlua::Value::Number(n) => {
            crate::vendor::lux::resp::write_integer(out, *n as i64);
        }
        mlua::Value::String(s) => {
            let b: Vec<u8> = s.as_bytes().to_vec();
            crate::vendor::lux::resp::write_bulk_raw(out, &b);
        }
        mlua::Value::Table(tbl) => {
            if let Ok(mlua::Value::String(s)) = tbl.get::<mlua::Value>("ok") {
                ...
                crate::vendor::lux::resp::write_simple(out, &sv);
                return;
            }
            if let Ok(mlua::Value::String(s)) = tbl.get::<mlua::Value>("err") {
                ...
                crate::vendor::lux::resp::write_error(out, &sv);
                return;
            }
            let len = tbl.len().unwrap_or(0) as usize;
            crate::vendor::lux::resp::write_array_header(out, len);
            for i in 1..=len {
...
fn create_redis_call(
    ...
    lua.create_function(move |lua_ctx, args: mlua::MultiValue| {
        ...
        let mut out = BytesMut::new();
        let lua_cache = std::sync::Arc::new(parking_lot::RwLock::new(
        ...
        crate::vendor::lux::cmd::execute_with_wal(
            &store, &lua_cache, &broker, &refs, &mut out, now,
        );
